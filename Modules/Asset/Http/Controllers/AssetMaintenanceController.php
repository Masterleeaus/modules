...
use Modules\Asset\Utils\AssetUtil;
...
class AssetMaintenanceController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($asset_maintenances)
                ...
                ->addColumn('warranty', function ($row) use ($now) {
                    ...
                    foreach ($row->asset->warranties as $w) {
                        ...
                        if ($now->between($start_date, $end_date)) {
                            ...
                            $html = '<span class="label bg-green">'.__('asset::lang.in_warranty').'</span><br>';

                            $html .= '<small>'.$this->commonUtil->format_date($w->start_date).' ~ '.$this->commonUtil->format_date($w->end_date).'</br>';
                            $html .= '('.$now->diffInDays($end_date, false).' '.__('asset::lang.days_left').') </small>';
                    ...
                    if (empty($warranty)) {
                        $html = '<span class="label bg-red">'.__('asset::lang.not_in_warranty').'</span>';
                    }

                    return $html;
                })
                ->editColumn('assigned_to_user', '@if(empty($assigned_user_id))<small class="label bg-lightgray text-danger">@lang("asset::lang.unassigned")</small> @else {{$assigned_to_user}} @endif')
                ...
                ->addColumn('action', function ($row) {
                    $html = '<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-primary edit_maintenance" data-href="'.action([\Modules\Asset\Http\Controllers\AssetMaintenanceController::class, 'edit'], [$row->id]).'"><i class="fas fa-edit"></i> '.__('messages.edit').'</button>';

                    $html .= ' <button type="button" data-href="'.action([\Modules\Asset\Http\Controllers\AssetMaintenanceController::class, 'destroy'], [$row->id]).'"  id="delete_asset_maintenance" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-error">
        ...
        return view('asset::asset_maintenance.index')->with(compact('statuses', 'priorities', 'users'));
    }
    ...
    public function create()
    {
        ...
        if (request()->ajax()) {
            ...
            return view('asset::asset_maintenance.create')
    ...
    public function store(Request $request)
    {
        ...
        return redirect()
            ->action([\Modules\Asset\Http\Controllers\AssetMaintenanceController::class, 'index'])
            ->with('status', $output);
    }
    ...
    public function edit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('asset::asset_maintenance.edit')
