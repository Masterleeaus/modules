...
use Modules\CleanQuality\Http\Requests\StoreSchedule;
use Modules\CleanQuality\Http\Requests\UpdateSchedule;
use Modules\CleanQuality\DataTables\SchedulesDataTable;
...
class ScheduleController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        $this->companyAddresses = $this->availableCompanyAddresses();
    ...
    public function show($id)
    {
        ...
        $this->viewPermission = ModuleAccess::permissionLevel('view_quality_control');
        $this->addSchedulesPermission = ModuleAccess::permissionLevel('add_quality_control');
    ...
    public function destroy($id)
    {
        ...
        $this->deleteCalendarEventForSchedule($schedule);
        $schedule->delete();
    ...
    public function domPdfObjectForDownload($id)
    {
        ...
        $template = data_get($this, 'scheduleSetting.template', 'invoice-1');
        $pdf->loadView('quality_control::schedules.pdf.' . $template, $this->data);
    ...
    public function domPdfObjectForConsoleDownload($id)
    {
        ...
        $locale = data_get($this, 'scheduleSetting.locale', app()->getLocale());
        App::setLocale($locale);
        Carbon::setLocale($locale);
        $pdf->loadView('quality_control::schedules.pdf.invoice-recurring', $this->data);
    ...
    {
        $editPermission = ModuleAccess::permissionLevel('edit_quality_control');
    ...
    protected function availableFloors()
    {
        return class_exists('Modules\Units\Entities\Floor')
            ? \Modules\Units\Entities\Floor::query()->get()
            : collect();
    }

    protected function availableTowers()
    {
        return class_exists('Modules\Units\Entities\Tower')
            ? \Modules\Units\Entities\Tower::query()->get()
            : collect();
    }

    protected function availableCompanyAddresses()
    {
        return class_exists(CompanyAddress::class)
            ? CompanyAddress::query()->get()
            : collect();
    }
