...
@section('filter-section')
    ...
    <div class="d-flex px-4 filter-box bg-white">
        ...
        @if ($viewComplaintPermission == 'all' || ($viewComplaintPermission == 'owned' && $complaint->agent_id == user()->id))
            <a href="javascript:;" class="d-flex align-items-center height-44 text-dark-grey text-capitalize border-right-grey px-3 resolve-complaint">
                <i class="fa fa-check-circle mr-0 mr-lg-2 mr-md-2"></i><span class="d-none d-lg-block d-md-block">Resolve</span>
            </a>

            @if (!empty($complaint->quality_control_id))
                <a href="javascript:;" class="d-flex align-items-center height-44 text-dark-grey text-capitalize border-right-grey px-3 schedule-followup">
                    <i class="fa fa-calendar-plus mr-0 mr-lg-2 mr-md-2"></i><span class="d-none d-lg-block d-md-block">Follow-up Inspection</span>
                </a>
            @endif
        @endif
...
$(document).ready(function () {
    $('.resolve-complaint').on('click', function () {
        var outcome = prompt('Resolution outcome (optional):', '');
        $.easyAjax({
            url: "{{ route('complaint.resolve', $complaint->id) }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
    ...
    $('.schedule-followup').on('click', function () {
        $.easyAjax({
            url: "{{ route('complaint.schedule_followup_inspection', $complaint->id) }}",
            type: "POST",
            data: { _token: "{{ csrf_token() }}" },
            success: function (res) {
                if (res.status === 'success') {
                    if (res.follow_up_schedule_id) {
                        // Try to open the follow-up schedule if route exists
                        try {
                            window.location.href = "{{ url('account/inspection_schedules') }}/" + res.follow_up_schedule_id;
                        } catch (e) {
                            window.location.reload();
                        }
                    } else {
