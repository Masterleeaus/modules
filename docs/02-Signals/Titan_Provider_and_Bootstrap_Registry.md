# Titan Provider and Bootstrap Registry

## Purpose

Defines how modules register themselves with Titan runtime during system boot.
...
Modules attach at provider phase only.

---

## Expanded Bootstrap Rules

Service providers and bootstrap registries determine what actually loads, in what order, and with which bindings. In a modular Titan/Worksuite stack, this has to be explicit.

### Provider responsibilities
Providers should register and boot:
- configs
- service container bindings
- routes
- views and translations where relevant
- migrations
- event listeners
- panel plugins or admin surfaces where appropriate
...
- module providers
- panel providers
- bridge providers for extension or route-loader compatibility

### Bootstrap registry goals
The registry should know:
- which providers exist
- whether their classes resolve
- whether they are enabled
- what dependencies they expect
- which runtime areas they serve

### Good patterns
- keep providers non-destructive
- bind interfaces to concrete services in providers, not ad hoc inside controllers
- use route loaders for modular route files
- centralize provider registration so Doctor can inspect it cleanly

### Anti-patterns
- business logic in provider boot methods
- duplicate route registrations in multiple places
- silent fallback when a provider or dependency is missing
- UI-only providers acting as the source of domain truth

---
## Bootstrap Order

A predictable bootstrap sequence reduces drift:

1. core platform providers
2. identity and tenancy providers
3. registry/discovery providers
4. module providers
5. surface providers such as CMS, PWA, Omni, Filament
6. optional bridge or compatibility providers

## Provider Responsibilities by Class

### platform providers
Bind shared contracts, registries, and engine services.

### module providers
Load module-local routes, views, migrations, translations, and bindings.

### panel/surface providers
Attach resources, widgets, channel adapters, or shell registrations.

## Acceptance Checklist

- provider class resolves
- dependencies are available
- boot method is non-destructive
- duplicate route registration does not occur
- provider can be inspected by Doctor
- provider failure is reported with a specific class or dependency message
