# Titan Scheduling Engine

## Purpose

Defines the canonical time-based execution layer for Titan.
This engine manages recurrence, delays, timers, execution windows, overlap control,
and scheduled handoff into automation, workflows, communications, and maintenance tasks.

---

## Canonical Scheduling Tree
...
## What the Scheduling Engine Owns

- recurring task definitions
- timer-based execution
- delayed follow-up execution
- reminder timing
- recurrence rules
- execution windows
- overlap prevention
- task hooks into engines and modules
- timing policy enforcement
- maintenance and background cadence control
...
### Scheduling Engine
Owns time and cadence.

### Automation Engine
Owns orchestration and execution policy.

The scheduling engine decides when something should wake up.

---

## Expanded Scheduling Design

The scheduling engine should formalize every timed behavior in the system: reminders, follow-ups, recurring jobs, maintenance windows, delayed communications, and overlap control.
...
### Responsibilities
- register scheduled tasks and timers
- model recurrence rules
- apply business windows and blackout periods
- prevent overlapping task runs when required
- emit due work into queues or automation pipelines
- track missed/late executions for observability

### Design rules
- Scheduler should dispatch work to queues/jobs rather than do heavy work inline.
- Time-zone and tenant rules must be explicit.
- Recurring jobs and reminders should be inspectable and re-playable.
- Use overlap controls for long-running maintenance and sync jobs.

### Typical uses
- reminder dispatch
- invoice follow-up timing
- rebooking cadence
- nightly reconciliation
- stale queue or sync maintenance
- recurring visit generation

---
## Time Invariants

The scheduling layer should preserve:

- explicit timezone interpretation
- recurrence definitions that can be inspected and re-run
- clear separation between “due now” and “missed or late”
- overlap policy per task family
- visibility into next-run and last-run state

## Integration Points

Scheduling should commonly hand off into:

- automation engines for orchestration
- communications for reminders and follow-ups
- workflow engines for due-state checks
- observability for missed-run alerts
- sync/runtime maintenance for offline node cleanup

## Anti-Patterns

Avoid placing complex business decisions directly in cron definitions or scheduler closures.
Use the scheduler to wake a named job, command, or orchestration entry point instead.
