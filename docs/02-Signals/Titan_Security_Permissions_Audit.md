# Titan Security, Permissions, and Audit

## Purpose

Defines the canonical security and audit model for Titan.
...
2. Company/tenant boundary enforcement
3. Explicit permission and role checks
...
- namespaced by module or system domain
- uniquely seeded
- attached to roles/capabilities
- consistent across UI, API, jobs, and AI tooling
- enforced at policy and runtime levels

---

## AI Security Rules

- Titan Zero is the sole AI authority
- Titan Core is the only execution router for providers
- modules must not call providers directly
- AI runs must be logged
- drafts must not silently overwrite final operational state
- no silent execution
- governance must classify risk before action

---

## Expanded Security Contract

### Tenant boundary
`company_id` is the tenant boundary for this stack. Every module, API, queue job, sync mutation, AI tool call, and communication action must respect that scope. `user_id` is actor context, not tenant identity.
...
- policy classes
- route middleware
- per-action approval rules for sensitive operations

### Sensitive action classes
Require stronger checks for:
- money movement
- customer communications on behalf of the business
- destructive deletes
- permission changes
- package/module enablement
- AI auto-execution of state-changing tools
...
- timestamp and execution reference

### Minimum review checklist
- routes protected by correct middleware
- policies exist for critical models/actions
- permissions seeded and named consistently
- no cross-tenant query paths
- API endpoints validate scope and actor
- background jobs preserve actor/tenant context where required
- secrets and provider credentials stay out of module code

---
## Approval-Aware Security

Not every sensitive action should be blocked; many should be routed through approval.
Examples:

- high-value invoice or money actions
- broadcast customer communications
- package/module changes
- AI-driven state-changing actions
- destructive deletes or irreversible closes
...
- execution reference or signal/tool reference

## Anti-Patterns

- relying on UI visibility instead of policy checks
- storing secrets in module code or Blade files
- allowing jobs or sync handlers to lose tenant context
- permitting AI to bypass the same policy boundaries humans must follow
