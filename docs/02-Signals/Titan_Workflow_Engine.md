...
Defines the formal workflow and state-machine runtime for Titan.
This engine controls multi-step progressions, guarded transitions, approvals, and reusable lifecycle templates.

---

## Canonical Workflow Tree
...
### Workflow Engine
Defines the structure of a multi-step process.

### Automation Engine
Determines when and how steps should be triggered, retried, escalated, or delayed.
...
- channel or module availability
- safety constraints

No transition should occur unless its guards pass.

---

## Expanded Workflow Design

### Purpose
The workflow engine models business lifecycles as explicit state machines with guards, approvals, and metrics. This is where the legal shape of a process should live.
...
- enforce guards and prerequisites
- integrate approvals where required
- track stuck or failed states
- expose reusable workflow templates to modules

### Relationship to modules
Modules should own their domain records and can expose workflow definitions, but the platform workflow engine provides the shared machinery for:
- state transitions
- approval holds
- transition auditing
- metrics and stuck-state detection

### Relationship to automation and scheduling
- **workflow** defines what can happen
- **automation** decides when to try to move it
- **scheduling** provides the timing window

### Implementation rules
- keep transition logic explicit and testable
- prefer named states and transitions over scattered booleans
- log every transition with actor, time, and reason where relevant
- support approval gates for high-risk transitions

---
## Transition Record

A workflow transition log should capture:

- workflow or definition key
- source state
- target state
- actor
- tenant
- reason or trigger
- approval reference if one existed
- timestamp
- failure code when transition was denied

## Template Use

Reusable templates should cover common operational families such as:

- lead to quote to booking
- draft to planned to scheduled to completed
- invoice issued to chased to paid
- issue raised to reviewed to resolved

Modules can specialize these templates, but the engine should keep the shared mechanics consistent.

## Anti-Patterns

- booleans scattered across models standing in for state machines
- transitions hidden in controller branches
- approvals implemented as ad hoc flags with no transition audit
- automation moving records into illegal states because guards were bypassed
